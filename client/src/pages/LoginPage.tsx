...
export default function LoginPage() {
  ...
  const mutation = useMutation({
    mutationFn: () => api.post('/auth/login', form).then((r) => r.data),
    onSuccess: (data) => {
      ...
      <div className="w-full max-w-md bg-white border border-gray-200 rounded-2xl shadow-sm p-8">
        ...
        {isAdmin && (
          <div className="mb-3 text-xs text-amber-700 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2">
            You are signing in as <strong>Admin</strong>. You'll be redirected to the admin dashboard.
          </div>
        )}
        ...
        <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); mutation.mutate(); }}>
          ...
          <div>
            ...
            <input className="input" type={showPassword ? 'text' : 'password'} required value={form.password}
              onChange={(e) => setForm({ ...form, password: e.target.value })} />
            <div className="mt-2 flex flex-col gap-3 text-sm text-gray-500 sm:flex-row sm:items-center sm:justify-between">
